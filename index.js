...
sub
.setName('list')
.setDescription('List permission roles')
),
...
option
.setName('channel')
.setDescription('Channel for case logs')
.setRequired(true)
)
)
.addSubcommand(sub =>
...
option
.setName('role')
.setDescription('Role to give while on duty')
.setRequired(true)
)
),
...
.addStringOption(option =>
option
.setName('suspect_username')
.setDescription('Roblox username or suspect name')
.setRequired(true)
)
.addStringOption(option =>
option
...
.addStringOption(option =>
option
.setName('arrest_location')
.setDescription('Where the arrest happened')
.setRequired(true)
)
...
if (interaction.isButton()) {
  ...
  if (action === 'start') {
    if (shift) {
      ...
      await run(
        `INSERT INTO active_shifts (userId, startedAt, status, breakStartedAt, totalBreakMs)
         VALUES (?, ?, ?, ?, ?)`,
...
if (command === 'welcome') {
  ...
  if (sub === 'set') {
    ...
    return interaction.reply({
      content: `✅ Welcome system set to ${channel}. Use /welcome test.`,
      ephemeral: true
    });
  }
...
if (command === 'autorole') {
  ...
  if (sub === 'add') {
    ...
    return interaction.reply({
      content: `✅ ${role} saved as an auto-role. Automatic join roles need Server Members Intent later.`,
      ephemeral: true
    });
  }
  ...
  if (sub === 'remove') {
    ...
    return interaction.reply({
      content: `✅ ${role} removed from auto-roles.`,
      ephemeral: true
    });
  }
...
if (command === 'perm') {
  ...
  if (sub === 'add') {
    ...
    return interaction.reply({
      content: `✅ Added ${role} to **${permission}** permissions.`,
      ephemeral: true
    });
  }
  ...
  if (sub === 'remove') {
    ...
    return interaction.reply({
      content: `✅ Removed ${role} from **${permission}** permissions.`,
      ephemeral: true
    });
  }
...
if (command === 'leaderboard') {
  ...
  const rows = await all(`
    SELECT userId, SUM(minutes) as total
    FROM shifts
    GROUP BY userId
    ORDER BY total DESC
    LIMIT 10
